...
tryCatch(qdb_remove(handle, name = "timeseries2"), error = function (err) {})
```
...
handle <- qdb_connect("qdb://127.0.0.1:2836")
qdb_ts_create(handle, name = "timeseries2",
     columns = c("column1" = column_type$blob, "column2" = column_type$double))
```
...
handle <- qdb_connect("qdb://127.0.0.1:2836")
qdb_attach_tags(handle, entry = "timeseries2", tags = "my_tag")
```
